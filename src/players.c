...
void player_init(struct player *_player){
	...
	_player->sprite_tank_down = (char*)malloc( (TANK_WIDTH * TANK_HEIGHT) * sizeof(char*));
	if(_player->sprite_tank_down == NULL){
		printf("Error creating player_buffer_sprites_data\n");	
	}
	
	_player->sprite_tank_left = (char*)malloc( (TANK_WIDTH * TANK_HEIGHT) * sizeof(char*));
	if(_player->sprite_tank_left == NULL){
		printf("Error creating player_buffer_sprites_data\n");	
	}
	
	_player->sprite_tank_right = (char*)malloc( (TANK_WIDTH * TANK_HEIGHT) * sizeof(char*));
