//===========================================================
// CAPITULO 19 - Desincronizacion: cazar el desvio
//
// El capitulo 18 funciona MIENTRAS las dos maquinas hagan las mismas
// cuentas. Este va de que pasa cuando no, y de como enterarse.
//
// Lo que se aprende aqui:
//
//   1. Que es una desincronizacion y por que es tan dificil de ver.
//   2. Que es un CHECKSUM de estado.
//   3. Que se mete dentro y, sobre todo, QUE NO.
//   4. Pulsa D y provoca una a proposito.
//
// Dos maquinas:  ./launch_game_both.sh  y en las dos  cd tutorial\ch19
//===========================================================
...
//===========================================================
// EL CHECKSUM DE ESTADO.
//
// Una desincronizacion es lo peor que te puede pasar en red, porque NO SE
// VE. Las dos partidas siguen corriendo, cada una convencida de ir bien, y
// simplemente van contando historias distintas. El jugador ve al otro tanque
// hacer cosas absurdas y no hay ningun error en ningun sitio.
//
// La unica forma de enterarse es comprobarlo: cada maquina resume su estado
// en un numero y lo compara con el de la otra. Si difieren, se separaron.
//
// Los primos (3, 5, 7, 11...) son para que el orden importe. Si sumaras
// todo a pelo, dos tanques intercambiados darian el mismo numero.
//
// QUE ENTRA: todo lo que las dos maquinas tienen que calcular igual.
// QUE NO ENTRA, y es igual de importante: nada que sea local de cada
// maquina. En el capitulo 21 apareceran camera_x y camera_y, que son
// legitimamente distintas en cada lado. Meterlas aqui daria una
// desincronizacion falsa en el frame 1 de todas las partidas.
//===========================================================
...
int main(){
	...
	printf("\n");
	printf("CAPITULO 19 - Desincronizacion\n");
	printf("\n");
	...
	printf("\n");
	printf("  D   = PROVOCAR una desincronizacion a proposito\n");
	printf("        (mueve tu tanque 1 pixel solo en ESTA maquina)\n");
	printf("  ESC = salir\n");
	printf("\n");
	...
	do {
		...
		//-----------------------------------------------
		// LA TECLA D: romperlo a proposito.
		//
		// Mueve el tanque UN pixel, solo en esta maquina. Un pixel. La otra
		// no se entera y sigue calculando sin el.
		//
		// A partir de ese frame las dos partidas son distintas, y a simple
		// vista NO SE NOTA todavia. Pero el checksum lo caza en la
		// siguiente comprobacion.
		//
		// Eso es lo que hace un checksum: convertir un fallo invisible en
		// una linea del log.
		//-----------------------------------------------
		...
		//-----------------------------------------------
		//-----------------------------------------------
		// Entregar el resumen de ESTE frame, y avanzar.
		//
		// lockstep.c lo manda con las teclas (van en el mismo paquete, asi
		// que es gratis) y lo compara con el que le llega de la otra
		// maquina unos frames despues.
		//-----------------------------------------------
		net_set_local_checksum(compute_state_checksum());
		net_advance_frame();

		if (net_desync_detected() == 1 && desync_seen == 0){
			desync_seen = 1;
		}
	...
	printf("\n");
	if (desync_seen == 1){
		printf("  DESINCRONIZACION DETECTADA.\n");
		printf("\n");
		printf("  Las dos maquinas dejaron de contar la misma historia, y el\n");
		printf("  checksum lo vio. Sin el, las dos habrian seguido tan\n");
		printf("  tranquilas mostrando partidas distintas.\n");
	}else{
		printf("  Sin desincronizaciones. Las dos maquinas fueron a la par\n");
		printf("  todo el rato.\n");
	}
	...
	printf("\n");
	printf("\n");
	printf("  El checksum no ARREGLA nada: solo avisa. Pero avisar es todo\n");
	printf("  lo que necesitas, porque te dice el frame exacto en que se\n");
	printf("  rompio y ahi es donde esta el fallo.\n");
	printf("\n");
